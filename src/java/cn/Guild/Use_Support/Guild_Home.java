...
public class Guild_Home {
    ...
    public boolean Set_Home(String Guild_name,Index index){
        Guild_Setup.Get_Guidl_Yaml(Guild_name).set(Guild_name+".Home",index.x+","+index.y+","+index.z);
    ...
    public Index Get_Home(String Guild_name){
        if(Guild_Home_Cache.containsKey(Guild_name))
            ...
            String[] list=  Guild_Setup.Get_Guidl_Yaml(Guild_name).getString(Guild_name+".Home").split(",");
            if(list.length==3){
